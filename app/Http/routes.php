...
$app->get('/api/post', function(Request $request){
  ...
  $to = $request->input('to');
    ...
    if(empty($request->input())){
      ...
      $results = app('db')->select(sprintf("SELECT * FROM wp_posts WHERE id=%s OR post_name=%s OR post_date BETWEEN %s AND NOW() OR post_date BETWEEN %s AND %s", $id, $slug, $from, $firts_time, $to));
    }
      return $results;
